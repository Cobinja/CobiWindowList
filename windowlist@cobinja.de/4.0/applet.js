...
class CobiAppButton {
	...
	constructor(workspace, applet, app) {
		...
		this._label = new St.Label();
		this._labelBox = new St.Bin({
			natural_width: 0, min_width: 0,
	...
	_updateLabel(actor, event) {
		...
		if (this._currentWindow && this._currentWindow.minimized && this._settings.getValue("button-bracket-on-minimized")) {
			text = "[" + text + "]";
		}
		this._label.set_text(text);
	...
	_onButtonRelease(actor, event) {
		...
		// middle mouse button
			...
			switch(this._settings.getValue("button-close-on-middle-click")){
				case 0:
					this._onClose();
					break;
				case 1:
	...
	_onClose() {
		this._windows[0].delete(global.get_current_time());
		return true;
	}
